...
PointLightSource::PointLightSource(
    ...
    : MeshObject(std::move(mesh), model_matrix),
      ...
      position(model_matrix.column(3)),
...
{
    pShader->bind();
    pShader->setUniformValue(("pointLights[" + std::to_string(index) + "].color").c_str(), color);
    pShader->setUniformValue(("pointLights[" + std::to_string(index) + "].position").c_str(), position);
    pShader->setUniformValue(("pointLights[" + std::to_string(index) + "].intensity").c_str(), intensity);
    pShader->setUniformValue(("pointLights[" + std::to_string(index) + "].constFactor").c_str(), constFactor);
    pShader->setUniformValue(("pointLights[" + std::to_string(index) + "].linFactor").c_str(), linFactor);
    pShader->setUniformValue(("pointLights[" + std::to_string(index) + "].quadFactor").c_str(), quadFactor);
}
